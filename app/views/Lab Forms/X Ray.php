...
$results = $medical->retrieveDataByDate("xray_table", array(
    'test_request_date', 'regNo', 'bht_no', 'surgeon', 'signature', 'stamp',
    'xray_no', 'xray_room', 'size', 'remarks', 'sign_radio', 'completed_date', 'region'
), $date, $regNo);
if ($results) {
    ...
    $stamp =  $results['stamp'];
    $xray_no =  $results['xray_no'];
    $xray_room = $results['xray_room'];
    $size = $results['size'];
    $remarks =  $results['remarks'];
    $sign_radio =  $results['sign_radio'];
    $completed_date = $results['completed_date'];
    $region = explode(',', $results['region']);
}
?>
...
<head>
    <meta charset="utf-8">
    <title>X-Ray Diagnostic Examination Results Form</title>
